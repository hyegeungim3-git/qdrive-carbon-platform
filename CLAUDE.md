...
| `운수사 경영 대시보드.dc.html` | 세운버스(주) 경영진용 — 손익 효과 수식: 연료비 1,065만(18.4tCO₂↔6,870L×1,550원/L) + 예지정비 420만 − 인센티브 310만 = 월 순 +1,175만원. 관제 coRanks(-5.8% 1위)와 정합. V2G 잠재 스트립: 전기 22대 × 14.4만 = 월 +317만 → 1,492만원 |
...
- **AI 에이전트 센터(dash A)**: 승인 대기는 `state.appr`(idx→ok/hold), 코파일럿은 `copilotQA()` 고정 시나리오 3건 + `_ask()`의 750ms 단계 타이머(`_chatTimers`, unmount 시 정리). HITL·설명가능성·정직한 시뮬레이션 3원칙은 기획서 §7.
- **코파일럿 라이브 AI 모드**: 자유 입력은 기본 데모 안내로 응답하고, '라이브 연결'로 사용자 본인의 Anthropic API 키를 넣으면 `_askLive()`가 `claude-opus-4-8`을 직접 호출(`anthropic-dangerous-direct-browser-access` 헤더). 시스템 프롬프트는 `_copilotSystem()` — 기획서 §4 정합 사전과 동일 수치. **키는 localStorage(`qdrive_api_key`) 전용, 코드·저장소 커밋 절대 금지.** Opus 4.8은 temperature 등 샘플링 파라미터 전송 시 400 — 넣지 말 것. 401/429/네트워크 오류는 채팅 안에 한국어로 표시.
- 목업 데이터 기준: 412대(경유 272·CNG 72·전기 68), 5개 운수회사, 배출계수는 환경부 고시.
- 기사앱 좌측 레일 하단에 관제 대시보드 복귀 링크(`관제`) 있음 — 데모 왕복 동선. 시민 페이지 푸터에도 관제 복귀 링크.
- **AI Planning 시뮬레이터 수식**: k번째 전환 차량의 연 감축 = 36.5 − 0.3k tCO₂ (AI가 효과순 선정 → 한계 체감). 누적 = `planCalc(n)` = 36.5n − 0.15n(n+1). 대당 실투자 1.1억(차량가 3.9억 − 보조금 2.8억), 연료비 절감 1.148백만원/tCO₂ (기존 'AI 추천' 카드의 6대=214t·ROI 2.7년과 정합). KOC 크레딧 8,900원/t. 슬라이더 변경 시 차트 재생성 — `_painted` 키가 `dash|period|planN|v2gN`.
- **V2G 시뮬레이터(dash P)**: 월 수익 = v2gN × 14.4만원 (대당 일 방전 80kWh × 차익 60원 × 30일), 피크 = v2gN × 0.1MW. c-v2g는 24시간 바(23~05시 충전 음수·14~17시 방전 양수, MWh) + 전력수요 라인 이중축. c-fly는 예지정비 적중률 62→91% 성장 곡선.
- **데이터 가치 콘텐츠**: 노선 관리에 정책 제안 3건(`policyProps` — 신호·정류장·전용차로, 담당 부서 연계), 기사앱 리포트에 퍼스널 인사이트 3종(취약 시간대·최고 조건·성장), 시민 페이지에 '이용이 바꾼 것들' 3건 — 데이터 축적 → 정책·서비스 개선 서사.
- 시민 페이지 소나무 환산: 6.6 kgCO₂/그루·년 → 연 누적 393.7t = 약 59,600그루.
- **시민 계산기 공유**: Web Share API → clipboard API → execCommand 3단 폴백 + 토스트. 합성 클릭(비신뢰 제스처)에서는 클립보드가 거부되므로 검증은 computer 도구의 실클릭으로.
- **다크 모드**: 토스 토큰이 라이트 전용이라 각 페이지 helmet `<style>`에서 시맨틱 별칭을 `html[data-theme=dark]`로 직접 오버라이드. `--card`/`--overlay`/`--chart-grid`는 자체 시맨틱 토큰. 인라인 `background:#fff`는 전부 `var(--card)`로 치환됨(지도 위 흰 라벨 칩만 `#ffffff`로 예외). 테마는 head의 부트스트랩 스크립트가 localStorage(`qdrive_theme`) → `prefers-color-scheme` 순으로 결정, 우하단 🌙/☀️ 플로팅 토글. 차트는 `_painted`/`_chartTheme` 키에 테마 포함으로 토글 시 재생성(다크 그리드 #242b35), 지도는 CARTO `dark_all` 타일로 재생성(`_mapTheme`). `grey-900` 리본은 이중 용도라 뒤집지 않음.
...
- 반응형 대응 — 관제 웹은 1280px+ 기준으로 설계됨 (태블릿 이하 미검증). 시민 페이지는 375/768 대응 완료
